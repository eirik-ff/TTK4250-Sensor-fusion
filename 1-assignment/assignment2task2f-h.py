...
def condition_mean(x, z, P, H, R):
    z_cov = H @ P @ H.T + R
    z_mean = H @ x
    cond = x + P @ H.T @ np.linalg.solve(z_cov, z - z_mean)
    return cond
...
def condition_cov(P, H, R):
    z_cov = H @ P @ H.T + R
    coeff = H @ P
    cond = P - coeff.T @ np.linalg.inv(z_cov) @ coeff
    return cond
...
ax2.axis('equal')
ax2.legend()
ax2.set_title("After conditioning (2f)")
plt.show(block=False)
...
ax3.axis('equal')
ax3.legend()
ax3.set_title("After both conditioning (2g)")
